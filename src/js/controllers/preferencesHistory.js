    ...
    $scope.csvHistory = function(cb) {
      ...
      getHistory(function(err, txs) {
        ...
        data.forEach(function(it, index) {
          ...
          _amount = (it.action == 'sent' ? '-' : '') + (amount * satToHUSH).toFixed(8);
          _note = it.message || '';
          _comment = it.note ? it.note.body : '';

          if (it.action == 'moved')
            _note += ' Moved:' + (it.amount * satToHUSH).toFixed(8)
          ...
          if (it.fees && (it.action == 'moved' || it.action == 'sent')) {
            var _fee = (it.fees * satToHUSH).toFixed(8)
            $scope.csvContent.push({
              'Date': formatDate(it.time * 1000),
              'Destination': 'Hush Network Fees',
              'Description': '',
              'Amount': '-' + _fee,
              'Currency': 'HUSH',
              'Txid': '',
