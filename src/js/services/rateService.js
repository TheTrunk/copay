...
var RateService = function(opts) {
  ...
  self.SAT_TO_HUSH = 1 / 1e8;
  self.HUSH_TO_SAT = 1e8;
...
RateService.prototype.updateRates = function() {
  ...
  function getHUSH(cb, tries) {
    tries = tries || 0;
    if (!self.httprequest) return;
    if (tries > 5) return cb('could not get HUSH rates');
...
RateService.prototype.toFiat = function(satoshis, code, chain) {
  ...
  return satoshis * this.SAT_TO_HUSH * this.getRate(code, chain);
};
...
RateService.prototype.fromFiat = function(amount, code, chain) {
  ...
  return amount / this.getRate(code, chain) * this.HUSH_TO_SAT;
};
